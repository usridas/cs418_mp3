 ...
 */
    ...
    */
    computeAABB(){
      var i = 0;
      for (i = 0; i<this.numVertices; i+=3)
      {
        if(this.vBuffer[i] < this.minXYZ[0])
        {
          this.minXYZ[0] = this.vBuffer[i];
        }
        if(this.vBuffer[i+1] < this.minXYZ[1])
        {
          this.minXYZ[1] = this.vBuffer[i+1];
        }
        if(this.vBuffer[i+2] < this.minXYZ[2])
        {
          this.minXYZ[2] = this.vBuffer[i+2];
        }
        if(this.vBuffer[i] > this.maxXYZ[0])
        {
          this.maxXYZ[0] = this.vBuffer[i];
        }
        if(this.vBuffer[i+1] > this.maxXYZ[1])
        {
          this.maxXYZ[1] = this.vBuffer[i+1];
        }
        if(this.vBuffer[i+2] > this.maxXYZ[2])
        {
          this.maxXYZ[2] = this.vBuffer[i+2];
        }
      }
    }
    ...
    loadFromOBJ(fileText)
    {
      ...
      for (var i = 0; i < lines.length; i++)
      {
          ...
          else
          {
              ...
              if (tokens[0] == "v")
              {
                this.vBuffer.push(parseFloat(tokens[1])); //x coordinate
                this.vBuffer.push(parseFloat(tokens[2])); //y coordinate
                this.vBuffer.push(parseFloat(tokens[3])); //z coordinate
                this.numVertices++;
              }
              else if (tokens[0] == "f")
              {
                this.fBuffer.push(parseInt(tokens[1] - 1)); //vertex 1
                this.fBuffer.push(parseInt(tokens[2] - 1)); //vertex 2
                this.fBuffer.push(parseInt(tokens[3] - 1)); //vertex 3
                this.numFaces++;
              }
          }
      }
